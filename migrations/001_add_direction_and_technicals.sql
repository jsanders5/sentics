...
-- Clear the stale pre-rewrite rows (duplicate symbols, no direction). The next
-- pipeline run fully repopulates the table, so nothing of value is lost. This
-- also removes the duplicate symbols that would otherwise block the unique index.
